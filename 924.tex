...
\begin{itemize}
    ...
    \item Théorème de \bsc{Löwenheim}-\bsc{Skolem}.
...
\begin{itemize}
    ...
    \item On peut montrer la décidabilité de l'arithmétique de
      ...
      données relationnelles. Les modèles de \bsc{Herbrand} mènent à la
...
%    Prendre n'importe quelle th complète, car Complète =>
%décidable. non. Oui, l'égalité sans symbole de fonction, avec F =
%forall x y. x=y
...
%incomplétude
  ...
  \item Le théorème de compacité est-il vrai si on se limite aux modèles finis ?
    % Non : On pose $$\theta_n := \exists x_1, \dots, x_n \left(\left(\bigwedge_{1\le i < j\le n}x_i \neq x_j \right) \bigwedge \left( \forall x \bigvee_{1\le i \le n} x = x_i\right) \right)$$ et on considère $$T := \{\neg \theta_n \mid n\in \N \}$$ alors pour tout $n$, $\mathcal{M} \vDash \theta_n$ ssi $|\mathcal{M}| = n$. En particulier tout sous ensemble fini de $T$ possède un modèle fini mais $T$ n'a aucun modèle fini.
\end{itemize}
